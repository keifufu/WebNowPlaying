...
const AdapterAuthors: Component<{ adapter: TAdapter }> = (props) => {
  ...
  return (
    <For each={props.adapter.authors}>
      {(author, i) => (
        <div
          class={clsx([theme() === "dark" && "text-zinc-400"], [theme() === "light" && "text-zinc-500"], [theme() === "konami" && "text-zinc-300"])}
        >
          <Show when={i() === 0}>
...
const Adapter: Component<{ adapter: TAdapter; enabled: boolean; info: SocketInfoState }> = (props) => {
  ...
  return (
    <div class={`mb-2 flex w-full flex-col items-center rounded-md border border-solid ${borderColorClass()} p-2`}>
      ...
      <div class={clsx("w-full overflow-hidden transition-all duration-200 ease-in-out", [props.enabled && "h-6"], [!props.enabled && "h-0"])}>
        <Show when={props.info._isPlaceholder}>
          <div>Loading..</div>
        </Show>
        <Show when={!props.info._isPlaceholder}>
          <div class="flex">
            <div class={clsx([props.info.isConnected && green()], [!props.info.isConnected && red()], [props.info.isConnecting && yellow()])}>
