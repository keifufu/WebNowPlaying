...
export const initPort = async () => {
  ...
  if (getCurrentSite() !== null) connect(id);
};
  ...
  try {
    ...
    switch (Events[type as keyof typeof Events]) {
      ...
      case Events.SETVOLUME:
        site.events.setVolume?.(parseInt(data));
        break;
      case Events.REPEAT:
        site.events.toggleRepeatMode?.();
        break;
      case Events.SHUFFLE:
        site.events.toggleShuffleActive?.();
        break;
      case Events.TOGGLETHUMBSUP:
        site.events.setRating?.(site.info.rating() === 5 ? 0 : 5);
        break;
      case Events.TOGGLETHUMBSDOWN:
        site.events.setRating?.(site.info.rating() === 1 ? 0 : 1);
        break;
      case Events.RATING:
        site.events.setRating?.(parseInt(data));
        break;
      default:
        break;
    }
