...
const site: Site = {
  ...
  info: {
    playerName: () => 'Kick',
    state: () => querySelectorReport<StateMode, HTMLVideoElement>('#video-holder video', (el) => (el.paused ? StateMode.PAUSED : StateMode.PLAYING), StateMode.PAUSED, 'state'),
    ...
    durationSeconds: () => querySelectorReport<number, HTMLVideoElement>('#video-holder video', (el) => {
    ...
    positionSeconds: () => querySelectorReport<number, HTMLVideoElement>('#video-holder video', (el) => {
    ...
    rating: () => 0,
    repeatMode: () => querySelectorReport<RepeatMode, HTMLVideoElement>('#video-holder video', (el) => (el.loop ? RepeatMode.ONE : RepeatMode.NONE), RepeatMode.NONE, 'repeatMode'),
    shuffleActive: () => false
  },
  events: {
    setState: (state) => querySelectorEventReport<HTMLVideoElement>('#video-holder video', (el) => (state === StateMode.PLAYING ? el.play() : el.pause()), 'setState'),
    skipPrevious: null,
    skipNext: null,
    setPositionSeconds: (seconds) => querySelectorEventReport<HTMLVideoElement>('#video-holder video', (el) => el.currentTime = seconds, 'setPositionSeconds'),
    setPositionPercentage: null,
    setVolume: (volume: number) => {
      querySelectorEventReport<HTMLVideoElement>('#video-holder video', (el) => {
        el.muted = false
