...
const site: Site = {
  ...
  info: {
    playerName: () => "Deezer",
    state: () =>
      querySelectorReport<StateMode, HTMLElement>(
        "(.player-controls svg)[1]",
        (el) => (el.getAttribute("data-testid") === "PauseIcon" ? StateMode.PLAYING : StateMode.PAUSED),
        StateMode.PAUSED,
        "state"
      ),
    ...
    shuffleActive: () =>
      querySelectorReport<boolean, HTMLElement>(
        "(.option-item svg)[2]",
        (el) => getComputedStyle(el).color === "rgb(239, 84, 102)",
        false,
        "shuffleActive"
      ),
  },
  ...
  events: {
    ...
    setPositionPercentage: (positionPercentage: number) => {
      querySelectorEventReport<HTMLInputElement>(
        ".slider-track-input",
        (el) => {
