...
const site: Site = {
  ...
  info: {
    ...
    repeatMode: () => querySelectorReport<RepeatMode, HTMLElement>('.group.loop-btn svg path', (el) => {
    ...
    shuffleActive: () => querySelectorReport<boolean, HTMLElement>('.group.loop-btn svg path', (el) => {
  ...
  events: {
    setState: (state) => {
      ...
      querySelectorEventReport<HTMLButtonElement>('.group.play-btn', (el) => el.click(), 'setState')
    },
    skipPrevious: () => querySelectorEventReport<HTMLButtonElement>('.group.prev-audio', (el) => el.click(), 'skipPrevious'),
    skipNext: () => querySelectorEventReport<HTMLButtonElement>('.group.next-audio', (el) => el.click(), 'skipNext'),
    ...
    toggleRepeatMode: () => {
      const click = (n = 1) => {
        for (let i = 0; i < n; i++) querySelectorEventReport<HTMLButtonElement>('.group.loop-btn', (el) => el.click(), 'toggleRepeatMode')
      }
      const shuffle = site.info.shuffleActive()
      const repeat = site.info.repeatMode()
      if (shuffle || repeat === RepeatMode.ONE) return click(2)
      click()
    },
    toggleShuffleActive: () => {
      const click = (n = 1) => {
        for (let i = 0; i < n; i++) querySelectorEventReport<HTMLButtonElement>('.group.loop-btn', (el) => el.click(), 'toggleShuffleActive')
      }
      const shuffle = site.info.shuffleActive()
      const repeat = site.info.repeatMode()
    ...
    setRating: (rating: number) => {
      ratingUtils.like(rating, site, {
        toggleLike: () => {
          querySelectorEventReport<HTMLButtonElement>('.player-content > button', (el) => el.click(), 'setRating')
        }
      })
    }
  }
