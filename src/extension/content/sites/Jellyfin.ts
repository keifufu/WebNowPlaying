...
const site: Site = {
  ...
  info: {
    ...
    rating: () =>
      querySelector<number, HTMLButtonElement>(
        ".nowPlayingBarUserDataButtons > button[data-isfavorite]",
        (el) => (el.getAttribute("data-isfavorite") === "true" ? 5 : 0),
        0
      ),
    repeatMode: () => {
...
const getPlayer = (): HTMLVideoElement | HTMLAudioElement | null =>
  document.querySelector<HTMLVideoElement>("video") || document.querySelector<HTMLAudioElement>("audio[src]");
