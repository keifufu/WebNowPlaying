...
const site: Site = {
  ...
  info: {
    ...
    state: () => (querySelector<StateMode, HTMLElement>('.player-controls__btn_play', (el) => (el.classList.contains('d-icon_play') ? StateMode.PAUSED : StateMode.PLAYING), StateMode.PAUSED)),
    ...
    repeatMode: () => querySelector<RepeatMode, HTMLButtonElement>('.player-controls__btn_repeat', (el) => {
    ...
    shuffleActive: () => querySelector<boolean, HTMLButtonElement>('.player-controls__btn_shuffle',
      (el) => el.classList.contains('player-controls__btn_on'),
      false)
  },
  events: {
    setState: (state) => querySelectorEvent<HTMLButtonElement>('.player-controls__btn_play', (el) => {
      el.click()
