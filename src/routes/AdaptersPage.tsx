    ...
    <div class='flex h-full w-full flex-col items-center'>
      <div class='w-full overflow-y-scroll'>
        <For each={BuiltInAdapters}>
          {(adapter) => (
            <Adapter name={adapter.name} enabled={!settings().disabledBuiltInAdapters.includes(adapter.name)} gh={adapter.gh} port={adapter.port} />
          )}
        </For>
        <For each={settings().customAdapters}>
          {(adapter) => (
            <CustomAdapter enabled={adapter.enabled} port={adapter.port} />
          )}
        </For>
